...
describe("Tho test Login functionality and navigate to Admin Tab", () => {
  ...
  it("Verify that the user is able to navigate to the Admin Tab & verify the page details & invoke the employee name for Admin", () => {
    cy.get("@adminpage").then(adminpage => {
      ...
      cy.get("div:nth-child(n+1) > div > div:nth-child(2) > div")
  ...
  it("Verify Edit Functionality", () => {
    ...
    cy.get("@adminpage").then(adminpage => {
      ...
      cy.get(".oxd-input-group .oxd-input")
        .first()
        .clearThenType(adminpage.updateUserName, { force: true })
      cy.get('.oxd-checkbox-wrapper [type="checkbox"]')
        .check({ force: true })
        .then(() => {
          cy.get(".oxd-input-group .oxd-input")
            .eq(1)
  ...
  it("Verify Top Menu", () => {
    navigateToAdminPanel()
    cy.get(".oxd-topbar-body-nav li span")
      .contains("Job")
      .click({ force: true })
      .then(() => {
